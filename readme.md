...
### **4.1.1 批量处理与进度条 (Batch processing & progress)**

本项目的 `main.py` 支持批量处理：当 `--in_raster` 指向一个目录时（非递归），脚本会遍历该目录下所有 `.tif` / `.tiff` 文件并逐个处理。

- mask 匹配规则：如果 `--mask` 指向一个目录，则脚本会尝试在该目录中查找与输入 raster 同名的掩膜文件（优先尝试完全同名 `basename.ext`，然后尝试 `basename + .tif`）。如果找不到相应掩膜，filter 阶段会报错并退出。可根据需要修改为跳过缺失掩膜的文件。
- 进度条：脚本会尝试使用 `tqdm` 显示处理进度（`tqdm.auto`），如果系统中未安装 `tqdm`，脚本将回退为普通的终端输出而不影响功能。
...
```bat
python main.py --in_raster edge_map --mask cropland --out_dir out_dir
```

如果想在 mask 不存在时跳过该 raster 而不是退出，请联系我或我可以为你修改脚本默认行为。
...
-----
  ...
  注意：`--in_raster` 除了可以传入单个 `.tif` 文件外，也可以传入包含多个 `.tif` 的目录（非递归）。当传入目录时，`main.py` 会按目录中每个 TIFF 文件逐个执行完整或指定阶段的处理。示例：
